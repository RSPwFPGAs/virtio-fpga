...
//    $fclose(file);
  ...
  always begin
	...
	if (`TOP_PATH.desc_queue_2.size() > 0) begin
          // read the whole descriptor chain, following the NEXT flag+next
          desc_entry_flg_next = 1'b1;
          desc_chain_len = 0;
          while (desc_entry_flg_next) begin
            // read from the input queue
            {desc_idx, desc_entry} = `TOP_PATH.desc_queue_2.pop_front();
            ...
            $display("th05 desc: %d, %d, %d, %d, %d", desc_entry_len, desc_entry_flg_next, desc_entry_flg_writ, desc_entry_flg_indi, `TOP_PATH.desc_queue_2.size());
          end

          // write to the output queue
          desc_chain_len = desc_entry_flg_writ? desc_chain_len: 0;//5.1.6.1
          `TOP_PATH.ring_used_queue_2.push_back({desc_idx, desc_chain_len});
        end

...
//    $fclose(file);
  ...
  always begin
	...
	if (`TOP_PATH.desc_queue[1].size() > 0) begin
          // read the whole descriptor chain, following the NEXT flag+next
          desc_entry_flg_next = 1'b1;
          desc_chain_len = 0;
          while (desc_entry_flg_next) begin
	  ...
	  // read from host mem@desc_entry_phy and write to the loopback queue
          ...
          proc_chain_phy = {(desc_entry_phy >> 2), 2'b00};
          @(posedge `CSR_PATH.clk);
	  `TOP_PATH.loopback_queue.push_back({2'b01, desc_chain_len});  // sop
	  while (proc_chain_len[31] != 1'b1) begin
	    ...
	    `TOP_PATH.loopback_queue.push_back({2'b11, data2[31: 0]});

            proc_chain_len = proc_chain_len - 4;
            proc_chain_phy = proc_chain_phy + 4;
          end
	  @(posedge `CSR_PATH.clk);
	  `TOP_PATH.loopback_queue.push_back({2'b10, 32'hBABEFACE});  // eop
